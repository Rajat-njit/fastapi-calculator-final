...
def test_get_engine_sqlite_branch(monkeypatch):
    from app import database
    monkeypatch.setattr(database.settings, "DATABASE_URL", "sqlite:///./test_cov.db")
    engine = database.get_engine()

    assert engine.url.get_backend_name() == "sqlite"
    assert engine.url.database.endswith("test_cov.db")
...
def test_get_db_session_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(database, "SessionLocal", mock_factory)
...
def test_init_models_runs():
    from app import database
    # No need to assert import calls; running this covers the lines
