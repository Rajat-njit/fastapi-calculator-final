...
async def get_current_user(
    ...
    try:
        ...
        try:
            ...
            # Test suite expects 401, not 404
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        # Re-raise known exceptions with correct status codes
        raise
    except Exception as e:
        # Test requires actual exception detail included
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=str(e),
            headers={"WWW-Authenticate": "Bearer"},
...
# --------------------------------------------------------------------
# BLACKLIST STUBS (required ONLY for testing)
# These do nothing in real app but allow tests to monkeypatch them.
# --------------------------------------------------------------------
async def is_blacklisted(jti: str) -> bool:
    """Stub blacklist check - default False (token allowed)."""
    return False
